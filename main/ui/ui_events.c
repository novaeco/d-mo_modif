...
void WIFIOPEN(lv_event_t* e) {
  ...
  // Signal WiFi task to start scanning
  xEventGroupSetBits(wifi_event_group, WIFI_SCAN_BIT);
}
...
void WIFIAPOPEN(lv_event_t* e) {
  ...
  if (ssid != NULL && strlen(ssid) > 0 && pwd != NULL && strlen(pwd) > 0 && channel != NULL && strlen(channel) > 0) {

    // If all fields are filled, initialize and open the AP
    wifi_open_ap();
    wifi_ap_init((uint8_t*)ssid, (uint8_t*)pwd, atoi(channel));

    // Signal WiFi task to handle SoftAP mode
    xEventGroupSetBits(wifi_event_group, WIFI_AP_BIT);
...
void WIFIAPCLOSE(lv_event_t* e) {
  // Clear the SoftAP handling bit
  xEventGroupClearBits(wifi_event_group, WIFI_AP_BIT);
