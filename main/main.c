 ...
 * | Info       :
 * |                UI Design：
 *                          1. User Login and Creation: Users can log in or create new accounts, and the created users
 *are saved to NVS, so data is not lost after power-down.
 ...
 *                          4. PWM: Can modify PWM output in multiple ways to control screen brightness. Additionally,
 *it can display information from a Micro SD card.
...
#include "config.h"       // Global configuration macros
...
void app_main() {
  ...
  BaseType_t task_ret = xTaskCreate(wifi_task, "wifi_task", WIFI_TASK_STACK_SIZE, NULL, 9, &wifi_TaskHandle);
  if (task_ret != pdPASS) {
    ESP_LOGE(TAG, "Failed to create WiFi task: %ld", task_ret);
    wifi_TaskHandle = NULL;
    return;
  }
}
