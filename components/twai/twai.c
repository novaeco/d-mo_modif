 ...
 *
 * This function stops the TWAI driver and then uninstalls it. It logs a success 
 * or failure message based on the result of the uninstallation process.
 *
 * @return None
 */
void can_uninstall()
{
    ...
    // Attempt to uninstall the CAN driver
    if(twai_driver_uninstall() == ESP_OK)
        // Log success if uninstallation was successful
        ESP_LOGI(CAN_TAG, "Driver uninstall");
    else
        // Log failure if uninstallation failed
        ESP_LOGI(CAN_TAG, "Failed to uninstall driver");
}
...
void can_write_Byte(twai_message_t message)
{
    ...
    {
        ESP_LOGI(CAN_TAG, "Failed to queue message for transmission");
    }
}
