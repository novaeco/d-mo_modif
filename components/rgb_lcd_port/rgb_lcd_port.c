 ...
 * | Info        :   RGB LCD driver code
 *----------------
 * |This version :   V1.1
 * | Date        :   2025-08-10
 * | Info        :   Minor fixes; comments; kept PSRAM framebuffers
 *
 ******************************************************************************/
...
esp_lcd_panel_handle_t waveshare_esp32_s3_rgb_lcd_init()
{
    ...
    (void)esp_lcd_rgb_panel_register_event_callbacks(panel_handle, &cbs, NULL); // Register event callbacks
...
void wavesahre_rgb_lcd_display_window(int16_t Xstart, int16_t Ystart, int16_t Xend, int16_t Yend, uint8_t *Image)
{
    ...
    uint8_t *dst_data = (uint8_t *)malloc((size_t)crop_width * crop_height * 2); // 2 bytes per pixel
    if (!dst_data) {
